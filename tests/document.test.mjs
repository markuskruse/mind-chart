...
const edges = [{ id: 'ab', type: 'border', source: 'a', target: 'b', label: 'relates to', selected: true, data: { sourceAnchor: { x: .3, y: 0, side: 'top' }, targetAnchor: { x: 0, y: .8, side: 'left' }, color: '#8f3029', lineStyle: 'dashed', thickness: 'thick' }, markerStart: { type: 'arrowclosed' }, markerEnd: { type: 'arrowclosed' } }];
...
test('round trips all document fields, excluding transient UI state', () => {
  ...
  assert.equal(restored.edges[0].markerStart.color, '#8f3029');
...
test('rejects malformed or unsupported documents before changing the editor', () => {
  ...
  for (const change of [
    ...
    (d) => { d.edges[0].data.color = '#abcdef'; },
    (d) => { d.edges[0].data.lineStyle = 'dotted'; },
    (d) => { d.edges[0].data.thickness = 'medium'; },
    (d) => { d.viewport.zoom = 0; },
  ]) {
